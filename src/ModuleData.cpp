...
bool ModuleData::intersection(const std::vector<std::string> & keys,
                                std::vector<std::string> & toDelete,
                                const SharemindModuleApi0x1SyscallContext * c)
{
    ...
    logger.debug() << "keydb_intersection ";
    const void * globalId = processFacility->globalId(processFacility);

    if (!globalId) {
        logger.debug() << "keydb_intersectio: no global id";
        globalIdSize = 0;
    }
    ...
    err = consensusFacility.blocking_propose(&consensusFacility,
                                             "keydb_intersection",
                                             proposalSize,
                                             proposal.get(),
                                             &conData);
